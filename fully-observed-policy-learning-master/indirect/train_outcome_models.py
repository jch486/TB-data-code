...
# trains and evaluates a predictive model for a single outcome (action) and model class 
def train_predictive_model_for_model_class(cohort_df, outcomes_df, outcome_col,
    ...
    # get parameter settings from the default hyperparameter grid if no parameter settings are provided
    if param_setting is None:
        grid = HyperparameterGrid()
        param_grid = grid.param_grids[model_class]
        # parameters is a list of the parameter combinations as dictionaries
    ...
    for i, param in enumerate(parameters):
        ...
        for trial in (range(num_trials)):
            ...
            if validate:
                # split_fn = split_cohort_abx for validation
                # split_cohort_abx: Given a DataFrame containing cohort features and a DataFrame containing outcome labels, 
                    # splits the features / labels data into train / validation sets on the basis of person ID. 
                    # This ensures that there are no individuals with data in both the training and validation sets.
                ...
                # defaults to train_test_split if a splitting function is not provided
                else:
        ...
        # update best_combo (the best combination of hyperparameters) and best_val_auc
...
# for a single outcome, tries multiple model classes and finds the best hyperparameters for each class
def get_best_params_by_model_class(cohort_df, outcomes_df,
    ...
    for model_class in model_classes:
        ...
        # doesn't have param_setting argument, so default hyperparameter grid will be used
...
# takes pre-selected best hyperparameters (from get_best_params_by_model_class) and
# re-trains models using more trials for more stable auc estimates
...
# trains the best model for each outcome across multiple splits 
# and predicts probabilities (that an outcome will occur for both train and validation cohorts
...
# chooses the best model class for each outcome based on aucs
def get_best_models(aucs_by_outcome):
    best_model_by_outcome = {}

    # aucs_by_outcome format = {outcome1: 
    #                           {model_class1: auc1, model_class2: auc2, ...},
    #                           outcome2: 
    #                           {model_class1: auc1, model_class2: auc2, ...}, 
    #                           ...}
...
def train_outcome_models_main(cohort_df, outcomes_df, results_path,
    ...
    if validate:
        ...
        for outcome_col in outcomes:
            ...
            # get the best hyperparameters for each model class

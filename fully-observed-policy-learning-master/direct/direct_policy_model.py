...
class DirectPolicyModel:
    ...
    def get_regularization_loss(self, reg_type, lambda_reg):
        ...
        # L1 regularization 
        # l1 regularization adds the sum of the absolute values of the parameters to the loss
        if reg_type == 'l1':
         ...
         # L2 regularization 
         # l2 regularization adds the sum of the squares of the parameters to the loss
        if reg_type == 'l2':
            for name, param in self.model.named_parameters():
                if 'bias' not in name:
    ...
    def train_policy(self, train_cohort, val_cohort,
        ...
        # set curr_training_params to training_params, default to default_training_params
    ...
    def get_mean_realized_reward(self, cohort_df, rewards_df):
        # get_actions() returns DataFrame with two columns: example_id, action
        # each row contains an example ID and the corresponding action chosen by the current
        # learned policy model
        cohort_actions = self.get_actions(cohort_df) 
        rewards_merged = rewards_df.merge(cohort_actions, on='example_id', how='inner')
        
        # x['action'] gives the column name corresponding to the action chosen
        # x[x['action']] retrieves the reward value from that column
        # apply(..., axis=1) does this for every row

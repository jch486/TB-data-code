...
def train_all_models(exp_name, num_trials,
    ...
    # runs trials with cohort split a different way each time (for validation)
    for trial in range(num_trials):
        logging.info(f"Starting trial {trial}")

        if validate:
            # run_direct_policy_training.construct_policy_frontier() is called with split_fn = split_cohort_abx
        ...
        # dictionary of reward parameter combinations mapped to models
...
def train_model(exp_name, trial,
    ...
    # return dictionary of reward parameter combinations mapped to models
    return models_dict
...
def evaluate_model(model_dicts_list, 
    ...
    # defaultdict(list) = dictionary where the values are lists
    all_stats = defaultdict(list)
    reward_param_names = sorted(reward_params[0].keys())
    metric_names = None

    # array where each row is one reward parameter combination
    ...
    # for each train/validation split, with its corresponding dictionary of models (one per reward setting)
    for train_val_split, model_dict in zip(train_val_splits, model_dicts_list):
        ...
        for cohort_name, cohort_df in train_val_split.items():
            ...
            for param_setting in reward_params:
                ...
                # get model-chosen actions for this cohort
                cohort_actions_df = current_model.get_actions(cohort_df)

                # merge actions with outcome data
                if cohort_name == 'test': 
    ...
    # combine parameter values with corresponding metrics (mean and standard deviations)

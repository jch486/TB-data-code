...
# defaults to false
# if not running evaluation on a validation set, features/outcomes for a separate test set are needed
# outcome model is tuned using validation data
...
# for both, required if using an indirect approach in test mode
...
parser.add_argument('--predictions_path',
                    type=str, 
                    help='Path to predictions generated from trained outcome prediction models')

# required if using validation
# map from each outcome to possible thresholds for that outcome
...
# required if evaluating on test set (no validation)
...
# required if using validation
# file specifies configuration for selecting optimal thresholds
...
# not required; parameters are learning rate, optimizer, etc.
...
# map from each parameter in reward function to values at which we want to learn a policy
# parameters for this case: omega, reward for deferring
...
if __name__ == '__main__':
    ...
    # If in test mode, load in test features / labels, along with optimal hyperparameters / models selected from validation  
    ...
    if args.mode == 'thresholding':
        ...
        # load threshold_selection_config from threshold_selection_config_path
